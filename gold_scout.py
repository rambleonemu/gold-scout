...
NOT_SOLID = re.compile(
    ...
    r"hge|electroplate|bonded|clad|over\s?sterling|over\s?silver|costume|fashion)\b", re.I)
...
def load_settings(cfg):
    ...
    for k in ("payout_pct", "trap_under_pct", "max_price", "max_detail_calls",
              "min_feedback_pct", "min_feedback_score"):
...
def evaluate_core(item, karat, grams, spot24, cfg, title_text=None, photos=None, gold_specific=False):
    ...
    verify = (not is_trap) and under_by >= 0.40
    ...
    return {
        ...
        "trap": is_trap, "verify": verify, "gold_wt": gold_specific,
        ...
        "seller_score": int(seller["feedbackScore"]) if seller.get("feedbackScore") is not None else None,
        "photos": photos,
        "id": item.get("itemId", ""),
...
def evaluate(item, spot24, cfg):
    ...
    if not karat or not grams:
        return None
    return evaluate_core(item, karat, grams, spot24, cfg, gold_specific=bool(gold_g))
...
def evaluate_deep(item, detail, spot24, cfg):
    ...
    return evaluate_core(item, karat, grams, spot24, cfg, title_text=title,
                         photos=photos, gold_specific=bool(gold_g))
